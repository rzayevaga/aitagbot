...
@rzayev.on_message(filters.command(["aitag","all", "tag", "marvel", "gelin", "gəl"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]))
...
async def help(client, message):
  text = '''
🔮 ╰__Gözəl, Əmrlər Aşağıdadır...__

🧸 **Əmrlər**:

» /aitag & aitag "səbəb": <i>Bütün üzvləri tağ edirəm.</i>
...
» /alive & alive: <i>İşləyib işləmədiyimi yoxla.</i>
'''
  await rzayev.send_message(message.chat.id, text, disable_web_page_preview=True)
...
async def get_id(client, message):
    try:
        ...
        if (not message.reply_to_message) and (message.chat):
            ...
            await message.reply(f"Yönləndirilmiş istifadəçi, {message.reply_to_message.forward_from.first_name} adlı istifadəçinin ID-si: <code>{message.reply_to_message.forward_from.id   }</code>.")
