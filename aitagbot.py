...
@rzayev.on_message(filters.command("help", "komek"))
...
» /all "input": <i>Bütün üzvləri tağ edirəm.</i>
...
async def get_id(client, message):
    try:
        ...
        if (not message.reply_to_message) and (message.chat):
            await message.reply(f"{message.from_user.first_name} adlı istifadəçinin ID-si <code>{message.from_user.id }</code>.\Bu söhbətin ID-si: <code>{message.chat.id}</code>.") 

        elif not message.reply_to_message:
            await message.reply(f"{message.from_user.first_name} userin ID'si <code>{message.from_user.id }</code>.") 
